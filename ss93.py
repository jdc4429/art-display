...
class Firework:
    def __init__(self):
        ...
        self.exploded = False
        self.particles = []  # Store the particles of the explosion
...
class Particle:
    ...
    def update(self):
        ...
        self.y += self.speed_y
        self.life -= 0.01
        self.alpha = int(self.life / 3 * 255)  # Reduce alpha over time

    def draw(self):
        # Create a Surface with per-pixel alpha to enable fading
...
class Firefly:
    ...
    def move(self):
        ...
        # Wrap around the screen if the firefly goes out of bounds
        if self.x < 0:
            self.x = screen_width
...
# Raindrop, snowflake, blizzard, special, and wave speed control variables
raindrop_speed = 8
raindrop_spawn_frequency = .95
snowflake_speed = 3
snowflake_spawn_frequency = 0.58
blizzard_speed = 10  # Adjust the blizzard speed
...
while running:
    ...
    if current_mode == "RAIN":
        ...
        if len(fireflies) < 100:  # Adjust the number of fireflies as needed
            firefly = Firefly()
