...
class STARFIELD:
    ...
    def generate_stars(self):
        ...
        for _ in range(self.num_stars):
            ...
            self.stars.append((x, y, brightness, angle, speed))

    def update(self):
        for i in range(len(self.stars)):
            x, y, brightness, angle, speed = self.stars[i]
            x += speed * math.cos(angle)
            y += speed * math.sin(angle)
            brightness -= 1  # Reduce brightness over time
            if brightness < 0:
                # Reset the star if it has faded out
                x = screen_width // 2
                y = screen_height // 2
                brightness = random.randint(128, 255)
                angle = random.uniform(0, 2 * math.pi)
                speed = random.uniform(1, 4)
            self.stars[i] = (x, y, brightness, angle, speed)

    def draw(self, screen):
        for x, y, brightness, _, _ in self.stars:
