...
class Heart:
    def __init__(self, x, y, speed, size='sml'):
        # Initialize a heart object with its position, speed, and size.
        # The size can be 'sml' (small), 'med' (medium), or 'lrg' (large).
        self.size = size
    ...
    def rise(self):
        # Move the heart shape upwards.
        self.rect.y -= self.speed
        if self.rect.bottom < 0:
            self.rect.top = screen_height

    def draw(self):
        # Draw the heart shape on the screen.
        screen.blit(self.image, self.rect)

class HEARTMAX:
    def __init__(self):
        # Initialize the HEARTMAX class, which manages multiple heart shapes.
        self.hearts = []
        ...
        # Create heart shapes of different sizes and add them to the list.
        # Adjust the number and frequency of hearts as desired.
        for _ in range(5):
            x = random.randint(0, screen_width)
            y = random.randint(0, screen_height)
            heart = Heart(x, y, random.uniform(1, 3), size='lrg')
            self.hearts.append(heart)

        # Create more medium-sized hearts.
        for _ in range(10):
            x = random.randint(0, screen_width)
            y = random.randint(0, screen_height)
            heart = Heart(x, y, random.uniform(1, 3), size='med')
            self.hearts.append(heart)

        # Create lots of small-sized hearts.
        for _ in range(30):
            x = random.randint(0, screen_width)
            y = random.randint(0, screen_height)
            heart = Heart(x, y, random.uniform(1, 3), size='sml')
            self.hearts.append(heart)

    def update(self):
        # Update the positions of all heart shapes (make them rise).
        for heart in self.hearts:
            heart.rise()

    def draw(self):
        # Draw all heart shapes on the screen.
        for heart in self.hearts:
            heart.draw()
